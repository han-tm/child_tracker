...
class MentorCreateTaskSetPhoto extends StatelessWidget {
  const MentorCreateTaskSetPhoto({super.key});

  void onPick(BuildContext context) async {
    final XFile? xfile = await CustomImagePicker.pickAvatarFromGallery(context);
    if (xfile != null && context.mounted) {
      context.read<MentorTaskCreateCubit>().onChangePhoto(xfile);
    }
  }
              ...
              child: Column(
                children: [
                  ...
                  Expanded(
                    child: EmojiPicker(
                      onPick: (emoji) {
                        context.read<MentorTaskCreateCubit>().onChangeEmoji(emoji);
                      },
                    ),
                  ),
                ],
