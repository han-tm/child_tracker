...
class MentorTaskEditPhotoScreen extends StatelessWidget {
  const MentorTaskEditPhotoScreen({super.key});

  void onPick(BuildContext context) async {
    final XFile? xfile = await CustomImagePicker.pickAvatarFromGallery(context);
    if (xfile != null && context.mounted) {
      context.read<MentorTaskEditCubit>().onChangePhoto(xfile);
    }
  }
                ...
                child: Column(
                  children: [
                    ...
                    Expanded(
                      child: EmojiPicker(
                        onPick: (emoji) {
                          context.read<MentorTaskEditCubit>().onChangeEmoji(emoji);
                        },
                      ),
                    ),
                  ],
