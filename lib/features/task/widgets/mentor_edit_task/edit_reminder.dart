...
class MentorTaskEditReminderScreen extends StatelessWidget {
  const MentorTaskEditReminderScreen({super.key});

  void onDatePick(BuildContext context) async {
    final selectedDate = await showDatePickerModalBottomSheet(context);
    if (selectedDate != null && context.mounted) {
      context.read<MentorTaskEditCubit>().onChangeReminderDate(selectedDate);
    }
  }
  ...
  void onTimePick(BuildContext context) async {
    ...
    if (selectedTime != null && context.mounted) {
      context.read<MentorTaskEditCubit>().onChangeReminderTime(selectedTime);
    }
  }
                ...
                Container(
                  ...
                  child: TabBar(
                    onTap: (value) {
                      ...
                      context.read<MentorTaskEditCubit>().onChangeReminderType(type);
                    },
                    dividerHeight: 0,
                              ...
                              return Row(
                                children: List.generate(numberOfCircles, (index) {
                                  ...
                                  return GestureDetector(
                                    onTap: () {
                                      context.read<MentorTaskEditCubit>().onChangeReminderDayTap(index);
                                    },
                                    child: Container(
