...
class BonusEditPhotoScreen extends StatelessWidget {
  const BonusEditPhotoScreen({super.key});

  void onPick(BuildContext context) async {
    final XFile? xfile = await CustomImagePicker.pickAvatarFromGallery(context);
    if (xfile != null && context.mounted) {
      context.read<EditBonusCubit>().onChangePhoto(xfile);
    }
  }
                ...
                child: Column(
                  children: [
                    ...
                    Expanded(
                      child: EmojiPicker(
                        onPick: (emoji) {
                          context.read<EditBonusCubit>().onChangeEmoji(emoji);
                        },
                      ),
                    ),
                  ],
